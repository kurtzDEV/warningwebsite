...
CREATE TABLE IF NOT EXISTS orders (
  ...
  qr_code_data TEXT,
  discord_link TEXT DEFAULT 'https://discord.gg/warningbypass',
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
DECLARE
  order_num TEXT;
BEGIN
  -- Gerar número do pedido com formato WB-YYYYMMDD-XXXX
  SELECT 'WB-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0')
  INTO order_num;
...
CREATE OR REPLACE FUNCTION register_purchase(user_uuid UUID, order_id UUID)
RETURNS VOID AS $$
BEGIN
  -- Atualizar estatísticas do usuário
  UPDATE user_stats
  SET total_purchases = total_purchases + 1,
      total_spent = total_spent + (SELECT total_amount FROM orders WHERE id = order_id),
      updated_at = NOW()
  WHERE id = user_uuid;
  
  -- Registrar atividade
  INSERT INTO user_activities (user_id, activity_type, activity_data)
  VALUES (user_uuid, 'purchase', jsonb_build_object('order_id', order_id, 'amount', (SELECT total_amount FROM orders WHERE id = order_id)));
END;
$$ LANGUAGE plpgsql;
...
CREATE POLICY "Users can view own order items" ON order_items
  FOR SELECT USING (auth.uid() = (SELECT user_id FROM orders WHERE id = order_items.order_id));

CREATE POLICY "Users can insert own order items" ON order_items
  FOR INSERT WITH CHECK (auth.uid() = (SELECT user_id FROM orders WHERE id = order_items.order_id));
...
-- Mensagem de sucesso
SELECT 'Schema do banco de dados criado com sucesso!' as status;
